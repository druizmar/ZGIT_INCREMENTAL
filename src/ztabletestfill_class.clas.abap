CLASS ztabletestfill_class DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ztabletestfill_class IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ZTABLETEST_GIT.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ZTABLETEST_GIT.
*   insert the new table entries
    INSERT ZTABLETEST_GIT FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ZTABLETEST_GIT INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
